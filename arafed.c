 ...
 * 
 * 
 * Modeleded after the IceCal stuff, so lots of this is hold over and needs to be cleaned out
...
int main(int argc, char **argv){
	...
	while(argc){ //okay, loop over the arguments
		if (strstr(*argv, "init")) { //if the command is to initialize the port
			...
			if ((retval = enableAtriCompoennts(auxFd, ext_i2c[3]))<0) { //try and enable this port, and throw an error if that fails
...
//more holdover from IceCal software
/*
...
//the old main from the IceCal software
/*
int main(int argc, char **argv) {
	int ace2_port = -1;
	int i;
