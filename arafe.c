...
int arafeWriteRegister(int auxFd, unsigned char reg, unsigned char value) {
	...
	int retval = writeToI2C(auxFd, ic2_addr, len, data); //write these things to I2C //the ic2_addr is defined in arafei2c.c, and is the I2C address of the ARAFE Master
...
int arafeReadRegister(int auxFd, unsigned char reg, unsigned char value){
	...
	data[0] = reg;
	int len = 1;
	int retval = writeToI2C(auxFd, ic2_addr, len, data); //write to I2C
	retval = readFromI2C(auxFd, ic2_addr | 0x1, len, data); //now read it
	//data[0] now has the return data
        std::cout<<"Register returned "<<data[0]<<std::endl; //so we print it out
        return retval; //return this value
}
